$.ajax({
  url:
    "https://api.nasa.gov/planetary/apod?api_key=emTQclHrP2HcT2EeB6iYywEtYk76s6WzUbtUdgFg",
  method: "get",
}).then((result) => {
  var img = result.url;
...
function myQuiz() {
  const myQuestions = [
    ...
    {
      Ques:
        " Which is/are the object(s) used for storing data on the client provided by the HTML local storage?",
      choices: [
  ...
  function renderQuestion() {
    ...
    for (let i = 0; i < myQuestions[currentQuestion].choices.length; i++) {
      ...
      answerEl.addEventListener("click", function () {
        if (
          (myQuestions[currentQuestion].userAnswer =
            myQuestions[currentQuestion].choices[i])
        ) {
          checkAnswer();
          nextQestion();
        }
  ...
  function renderLocalStorage() {
    ...
    gameEndStorageDiv.setAttribute("class", "display-1 abc");
    ...
    yourRecordDiv.innerHTML = "Enter your name: ";

    var myResInput = document.createElement("input");
    myResInput.setAttribute("class", "input-group myInput");
    myResInput.setAttribute("id", "intial-input");
    myResInput.setAttribute("type", "name");
    myResInput.setAttribute("placeholder", "Enter Nsme");

    var submitHighscoreBtn = document.createElement("button");
    submitHighscoreBtn.setAttribute("class", "btn btn-success btn-sm");
    submitHighscoreBtn.setAttribute("id", "btn-submit");
    submitHighscoreBtn.innerHTML = "Save Score";

    yourRecordDiv.append(myResInput, submitHighscoreBtn);

    gameEndStorageDiv.append(userScoreMessageEl);
    gameEndStorageDiv.append(yourRecordDiv);
    //gameEndStorageDiv.append();
    //generateRowCol(1, gameEndStorageDiv);
    document.body.append(gameEndStorageDiv);
  ...
  function highscoreRecord(highscores) {
    ...
    highscoreContainerDiv.setAttribute("class", "container mydiv");
    //<div class="personal" id="main-container">
    var personal = document.createElement("div");
    personal.setAttribute("class", "personal");
    //personal.setAttribute("id", "")
    highscoreContainerDiv.appendChild(personal);
    ...
    personal.append(highscoreHeaderEl);

    var highschordisplayDiv = document.createElement("div");
    personal.append(highschordisplayDiv);
    if (highscores) {
...
function dragElement(a) {
  ...
  var f = 0,
    ...
    h = 0,
    i = 0;
  document.getElementById(a.id + "header")
    ? (document.getElementById(a.id + "header").onmousedown = b)
    : (a.onmousedown = b);
}
